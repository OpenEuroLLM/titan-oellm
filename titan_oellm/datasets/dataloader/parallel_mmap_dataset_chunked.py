...
class ChunkedMMapDataset(torch.utils.data.IterableDataset):
    ...
    def __init__(
        ...
        logger.info(f"Worker {dp_rank}: Assigned {len(self.worker_chunks)} chunks, "
                    f"total samples: {self.total_worker_samples}")
    ...
    def assign_chunks_to_worker(self, all_chunks: List) -> None:
        ...
        if (total_chunks // self.dp_world_size) * self.dp_world_size != total_chunks:
            logger.warning("Dataloader: Wolrd size is not 2^x, unequal many chunks per worker! Gradient Accumulation with different lbs/worker will not prvide the same results. ")
        ...
        for chunk_path in self.worker_chunks:
            try:
                ...
                self.total_worker_samples += chunk_length
    ...
    def load_chunks(self) -> None:
        """Load the all chunks for reading"""

        # Clean up previous chunk if exists
        if hasattr(self, 'idx_readers'):
            del self.idx_readers
        if hasattr(self, 'bin_readers'):
            del self.bin_readers

        local_rng = np.random.RandomState(seed=self.seed + self.epoch_counter)

        self.worker_chunks = local_rng.permutation(self.worker_chunks).tolist()
    ...
    def __next__(self):
        ...
        if self.current_worker_samples >= self.total_worker_samples:
            if self.infinite:
        ...
        chunk = self.current_worker_samples % len(self.idx_readers)

        sequence = self.read_sequence_at_position(chunk)

        # Update position
        self.current_worker_samples += 1
        self.sample_counter += 1

        return {'tokens': np.array(sequence)}

    def read_sequence_at_position(self, chunk: int) -> numpy.ndarray:
        """Read sequence at specific position in current chunk"""

        position = self.chunk_positions[chunk]
        if position >= self.chunk_lengths[chunk]:
            self.chunk_positions[chunk] = 0
            position = 0

        sequence_pointer, sequence_length, sequence_mode = self.idx_readers[chunk][position]
        sequence = self.bin_readers[chunk].read(
            dtype=self.idx_readers[chunk].dtype,
            count=sequence_length,
            offset=sequence_pointer
        )
        self.chunk_positions[chunk] += 1
    ...
    def __getstate__(self) -> Dict[str, Any]:
        ...
        state.pop('idx_readers', None)
        state.pop('bin_readers', None)
        return state
    ...
    def __setstate__(self, state: Dict[str, Any]) -> None:
        ...
        self.chunk_positions = state['chunk_positions']           
        self.total_worker_samples = state['total_worker_samples']
        self.current_worker_samples = state['current_worker_samples']
