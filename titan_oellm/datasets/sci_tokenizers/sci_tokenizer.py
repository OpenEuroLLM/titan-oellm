...
"""
Custom HuggingFace tokenizer wrapper with configurable BOS/EOS handling.

NOTE: This implementation is based on torchtitan v0.2.0's HuggingFaceTokenizer
and adds custom configuration support via job_config.sci_tokenizer section.
Most of the tokenizer loading and inference logic is identical to torchtitan's
implementation. The key difference is support for runtime config overrides of
default_add_bos and default_add_eos via the [sci_tokenizer] TOML section.
"""
...
class SciHFTokenizer(BaseTokenizer):
    """
    ...
    def __init__(self, tokenizer_path: str):
        ...
        # Set TorchTitan required attributes
    ...
    def encode(
        ...
        *,
        ...
        if not self.hf_adds_bos and bos:
            if hasattr(self, 'bos_id') and self.bos_id is not None:
                token_ids.insert(0, self.bos_id)

        # Add EOS token if requested and not already added by tokenizer
        if not self.hf_adds_eos and eos:
            if hasattr(self, 'eos_id') and self.eos_id is not None:
...
def build_sci_hf_tokenizer(job_config: JobConfig) -> SciHFTokenizer:
    """
    Builds a SciHFTokenizer from the specified path.

    This function creates a SciHFTokenizer instance that handles BOS/EOS token
    ...
    Args:
        job_config: A JobConfig object containing the path to the tokenizer directory.
    ...
    if hasattr(job_config, 'sci_tokenizer'):
        tokenizer.default_add_bos = job_config.sci_tokenizer.default_add_bos
        tokenizer.default_add_eos = job_config.sci_tokenizer.default_add_eos

        # Override HF tokenizer behavior if requested
        if job_config.sci_tokenizer.override_hf_bos_behavior:
            tokenizer.hf_adds_bos = False
        if job_config.sci_tokenizer.override_hf_eos_behavior:
            tokenizer.hf_adds_eos = False

        logger.info(
            f"SciTokenizer config applied: default_add_bos={tokenizer.default_add_bos}, "
            f"default_add_eos={tokenizer.default_add_eos}"
        )
