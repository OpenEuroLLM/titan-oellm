"""
Bounded Spherical Adam + Muon Gradient Direction.

Extends BoundedSphericalAdam with Muon-style gradient orthogonalization
(Newton-Schulz on projected gradient before Adam) — gives isotropic
gradient direction with Adam's adaptive step sizes.
...
Step order:
  1. Pre-step: project gradients/momentum to tangent space (radial removal)
  2. Muon NS: orthogonalize gradient direction, preserve magnitude (if enabled)
  3. Fused AdamW step (adaptive per-element step sizes)
  4. Post-step: enforce constraint
...
_shared_bound_muon_config: dict = {
    "betas": (0.9, 0.95),
    "eps": 1e-8,
    "weight_decay": 0.0,
    "mode": "bounded",
    ...
    "muon_ns_steps": 5,
    "muon_ns_mode": "muon",
    "muon_preserve_norm": True,
    "muon_ns_dtype": "bfloat16",
}


def configure_shared_bound_muon(
    ...
    """Configure SharedBoundMuon settings used by make_build_shared_bound_muon_optimizers().
    ...
    Args:
        ...
        muon_on_gradient: Apply Muon-style Newton-Schulz orthogonalization to the
            projected gradient before Adam. Gives isotropic direction + adaptive step.
        muon_ns_steps: Newton-Schulz iterations for gradient orthogonalization (default: 5).
        muon_ns_mode: NS coefficient mode: "muon" (fast), "convergent", or "cubic".
    """
    _shared_bound_muon_config["betas"] = betas
    _shared_bound_muon_config["eps"] = eps
    _shared_bound_muon_config["weight_decay"] = weight_decay
    _shared_bound_muon_config["mode"] = mode
    _shared_bound_muon_config["max_norm"] = max_norm
    _shared_bound_muon_config["project_gradients"] = project_gradients
    _shared_bound_muon_config["soft_blend"] = soft_blend
    _shared_bound_muon_config["rotation_lr"] = rotation_lr
    _shared_bound_muon_config["fallback_lr"] = fallback_lr
    _shared_bound_muon_config["embedding_lr"] = embedding_lr
    _shared_bound_muon_config["n_iter_spectral"] = n_iter_spectral
    _shared_bound_muon_config["n_iter_ns"] = n_iter_ns
    _shared_bound_muon_config["n_iter"] = n_iter
    _shared_bound_muon_config["ffn_down_left_ns"] = ffn_down_left_ns
    _shared_bound_muon_config["ns_alpha"] = ns_alpha
    _shared_bound_muon_config["ns_mode"] = ns_mode
    _shared_bound_muon_config["kappa_target"] = kappa_target
    _shared_bound_muon_config["lambda_max"] = lambda_max
    _shared_bound_muon_config["kappa_ema_beta"] = kappa_ema_beta
    _shared_bound_muon_config["ns_schedule_steps"] = ns_schedule_steps
    _shared_bound_muon_config["out_norm_dim_0"] = out_norm_dim_0
    _shared_bound_muon_config["muon_on_gradient"] = muon_on_gradient
    _shared_bound_muon_config["muon_ns_steps"] = muon_ns_steps
    _shared_bound_muon_config["muon_ns_mode"] = muon_ns_mode
    _shared_bound_muon_config["muon_preserve_norm"] = muon_preserve_norm
    _shared_bound_muon_config["muon_ns_dtype"] = muon_ns_dtype
...
def make_build_shared_bound_muon_optimizers():
    """
    Factory function that creates a build_optimizers_fn for SharedBoundMuon.

    Uses module-level configuration set via configure_shared_bound_muon().

    Optimizer groups (all handled by a single fused AdamW per model part):
      - 2D hidden matrices: fused AdamW + pre-step projection + Muon NS + post-step constraint
    ...
    def build_bounded_spherical_adam_optimizers_fn(
        ...
        if optimizer_config.early_step_in_backward:
            raise NotImplementedError(
                "SharedBoundMuon does not support early_step_in_backward."
            )

        fallback_lr = _shared_bound_muon_config.get("fallback_lr")
        if fallback_lr is None or fallback_lr <= 0:
            fallback_lr = optimizer_config.lr

        embedding_lr = _shared_bound_muon_config.get("embedding_lr")
        if embedding_lr is None or embedding_lr <= 0:
            embedding_lr = fallback_lr

        return SharedBoundMuonOptimizerContainer(
            model_parts=model_parts,
            constrained_lr=optimizer_config.lr,
            embedding_lr=embedding_lr,
            scalar_lr=fallback_lr,
            bounded_spherical_config=_shared_bound_muon_config,
        )

    return build_bounded_spherical_adam_optimizers_fn


def _newton_schulz_gradient(
    M: torch.Tensor,
    steps: int = 5,
    mode: str = "muon",
    eps: float = 1e-8,
    ...
    """Newton-Schulz iteration for approximate orthogonalization of gradients.

    Used for Muon-style gradient direction orthogonalization.
    Handles rectangular matrices by transposing so rows <= cols.

    Args:
        M: Gradient matrix (d_out, d_in).
        steps: Number of NS iterations (default: 5).
        mode: Coefficient mode — "muon" (fast), "convergent", or "cubic".
        eps: Numerical stability epsilon.
        ns_dtype: Dtype for NS iterations (default bfloat16). Pass torch.float32
            to test whether bf16 precision loss affects long-run performance.
    """
    if mode == "muon":
    ...
    # Normalize in fp32 for stability, then cast to ns_dtype for iterations
    X = M.float()
    X = (X / (X.norm() + eps)).to(ns_dtype)
    for _ in range(steps):
        A = X @ X.T  # (rows, rows) Gram — smaller when rows <= cols
        X = a * X + b * (A @ X) + c * (A @ (A @ X))
...
def _newton_schulz_gradient_batched(
    M: torch.Tensor,
    steps: int = 5,
    mode: str = "muon",
    eps: float = 1e-8,
) -> torch.Tensor:
    """Batched Newton-Schulz for a stack of rectangular gradient matrices.

    All matrices in the batch must have the same shape. Uses left-gram form
    with torch.baddbmm for fused multiply-add (matching cayley_optimizer's
    newton_schulz_batched pattern). Normalizes in fp32, iterates in bf16.

    Args:
        M: Stack of gradient matrices, shape (batch, rows, cols).
        steps: NS iterations (default: 5).
        mode: Coefficient preset — "muon" (fast), "convergent", or "cubic".
        eps: Per-matrix normalization epsilon.
    """
    if mode == "muon":
        a, b, c = 3.4445, -4.7750, 2.0315
    elif mode == "convergent":
    ...
    # Normalize per-matrix in fp32 for stability, iterate in bf16 for speed
    X = M.float()
    norms = X.norm(dim=(-2, -1), keepdim=True).clamp(min=eps)
    X = (X / norms).to(torch.bfloat16)

    # Batched NS — same operation order as unbatched: A@X then A@(A@X)
    # Avoids computing gram² which amplifies bf16 rounding errors.
    # NOTE: baddbmm variant below is ~10% faster but less precise in bf16:
    #   gram_update = torch.baddbmm(gram * b, gram, gram, alpha=c)  # b*gram + c*gram²
    #   X = torch.baddbmm(X * a, gram_update, X)                    # a*X + gram_update@X
    for _ in range(steps):
        A = torch.bmm(X, X.transpose(-2, -1))    # (B, rows, rows) gram
        AX = torch.bmm(A, X)                      # (B, rows, cols)  A@X
        X = a * X + b * AX + c * torch.bmm(A, AX) # a*X + b*(A@X) + c*(A@(A@X))
...
class SharedBoundMuonOptimizerContainer(OptimizersContainer):
    """
    ...
    def __init__(
        ...
        # Muon gradient direction config
        self._muon_on_gradient = bounded_spherical_config.get("muon_on_gradient", False)
        self._muon_ns_steps = bounded_spherical_config.get("muon_ns_steps", 5)
        self._muon_ns_mode = bounded_spherical_config.get("muon_ns_mode", "muon")
        self._muon_preserve_norm = bounded_spherical_config.get("muon_preserve_norm", True)
        _ns_dtype_str = bounded_spherical_config.get("muon_ns_dtype", "bfloat16")
        self._muon_ns_dtype = torch.float32 if _ns_dtype_str == "float32" else torch.bfloat16
        ...
        # Build shape groups for batched Muon NS gradient orthogonalization
        if self._muon_on_gradient:
            muon_eligible_per_opt = []
            for norm_ps, bound_ps, po_ps, pob_ps in zip(
                ...
                muon_eligible_per_opt.append(eligible)
            self._muon_shape_groups = self._build_shape_groups(muon_eligible_per_opt)
        else:
            self._muon_shape_groups = []
        ...
        logger.info(
            f"SharedBoundMuonOptimizerContainer created:\n"
            ...
            f"  Muon gradient: muon_on_gradient={self._muon_on_gradient}, "
        ...
        for opt_idx, shape_groups in enumerate(self._muon_shape_groups):
            if shape_groups:
                group_info = ", ".join(
                    f"{s}: {len(ps)} params" for s, ps in shape_groups.items()
                )
                logger.info(f"  Muon NS shape groups (part {opt_idx}): {group_info}")
    ...
    @torch.no_grad()
    def _apply_muon_to_projected_gradients(self):
        """Apply Muon-style NS orthogonalization to projected gradients.

        All-gathers each gradient once to get the full matrix, runs NS on the
        full plain tensor (correct global Gram, no per-iteration all-reduces),
        then writes back the local shard. This is 1 all-gather per param
        instead of ~15 implicit all-reduces (5 iters × 3 matmuls).

        Preserves per-param gradient magnitude for Adam's adaptive step sizes.
        """
        for opt_idx, shape_groups in enumerate(self._muon_shape_groups):
            for shape_key, group in shape_groups.items():
                for p in group:
                    ...
                    # All-gather full gradient once (single comm op)
                    full_grad, shard_info = self._all_gather_grad(p)

                    orig_norm = full_grad.float().norm()
                    if orig_norm < 1e-12:
                        continue

                    # NS on full plain tensor — no DTensor, no implicit all-reduces
                    g_ortho = _newton_schulz_gradient(
                        full_grad,
                        steps=self._muon_ns_steps,
                        mode=self._muon_ns_mode,
                        ns_dtype=self._muon_ns_dtype,
                    )

                    if self._muon_preserve_norm:
                        # Rescale NS output to preserve original gradient Frobenius norm.
                        # This is the default BSM behaviour — Adam's v_t then tracks
                        # the original per-param gradient magnitude across training steps.
                        ortho_norm = g_ortho.float().norm().clamp(min=1e-8)
                        result = (g_ortho.float() * (orig_norm / ortho_norm)).to(full_grad.dtype)
                    else:
                        # No norm-preservation: Adam sees the raw NS direction, whose
                        # Frobenius norm ≈ sqrt(min(R,C)) is nearly constant.
                        # v_t will converge faster to a constant → closer to SGD.
                        result = g_ortho

                    # Write back local shard
                    self._write_back_grad_shard(result, p, shard_info)

    def step(self, *args, **kwargs):
        """Pre-step projection -> Muon NS -> fused Adam step -> constraint enforcement."""
        # 1. Pre-step: project gradients and momentum for constrained params
        if self._project_gradients:
            self._pre_step_projections()

        # 1b. Muon-style gradient direction orthogonalization
        if self._muon_on_gradient:
            self._apply_muon_to_projected_gradients()
    ...
    def _all_gather_grad(self, p: torch.nn.Parameter):
        """All-gather full gradient for NS, return (full_grad, shard_info).

        Same pattern as _get_full_weight() but operates on p.grad.
        Returns (full_grad, None) for plain tensors, or
        (full_grad, (local_grad_tensor, offset, local_size)) for DTensors.
        """
        try:
            from torch.distributed._tensor import DTensor
            is_dtensor = isinstance(p.grad, DTensor)
        except ImportError:
            is_dtensor = False
        ...
        dt = p.grad
        full_G = dt.full_tensor()
        local_tensor = dt._local_tensor
        local_size = local_tensor.shape[0]
        full_size = full_G.shape[0]
        ...
        return full_G, (local_tensor, offset, local_size)

    def _write_back_grad_shard(
        self, result: torch.Tensor, p: torch.nn.Parameter, shard_info,
    ):
        """Write NS result back to local grad shard."""
        if shard_info is None:
            p.grad.copy_(result)
        else:
            local_tensor, offset, local_size = shard_info
