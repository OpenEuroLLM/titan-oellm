...
# All rights reserved.
#
# Universal OU Stochastic Learning Rate Scheduler

"""
Universal OU Stochastic LR Scheduler - Unified Scheduler for All Use Cases

This module implements a universal learning rate scheduler that can emulate all other
schedulers through a flexible three-phase architecture with optional OU stochastic process.
...
2. Phase 2 (Main): Base schedule with optional OU stochastic process and configurable decay
...
- OU parameters: ou_theta, ou_sigma, ou_max_change, ema_alpha, ou_seed
...
Example (replaces old OU with warmdown):
...
class UniversalOUStochasticLR(_LRScheduler):
    """
    Universal OU Stochastic Learning Rate Scheduler.

    A unified scheduler that can replace all other LR schedulers through flexible
    three-phase architecture with optional OU stochastic process.
    ...
    Args:
        ...
        # Phase 2 - Main
        use_ou_process (bool): Enable OU stochastic process. Default: False
        ...
        ou_theta (float): OU mean reversion rate. Lower = smoother. Default: 0.008
        ou_sigma (float): OU noise amplitude around 1.0. Default: 0.1
        ou_max_change (float): Maximum OU factor change per step. Default: 0.05
        ema_alpha (float): EMA smoothing factor (0-1). Higher = smoother. Default: 0.99
        seed (int): Random seed for OU process reproducibility. Default: 1
    ...
    def __init__(
        ...
        self.use_ou_process = use_ou_process
        ...
        self.ou_max_change = ou_max_change
        self.ema_alpha = ema_alpha

        # Initialize dedicated RNG for OU process
        self.ou_seed = seed
        self.rng = random.Random(self.ou_seed)
        ...
        self._ou_factors = None
        self._ema_ou_factors = None
    ...
    def _compute_main_base_lr(self, base_lr, t_main, main_decay_ratio=None):
        """
        ...
        Args:
            ...
            main_decay_ratio: Optional per-group override for self.main_decay_ratio

        Returns:
            Base schedule learning rate (before OU multiplication if enabled)
        """
        if self.main_decay_type == "const":
            return base_lr

        if main_decay_ratio is None:
            main_decay_ratio = self.main_decay_ratio
        ...
        target_lr = max(self.lr_min_absolute, base_lr * main_decay_ratio)
    ...
    def get_lr(self):
        ...
        if self._ou_factors is None:
            # Initialize OU factors to 1.0 (multiplicative identity)
            self._ou_factors = [1.0] * len(self.base_lrs)
            self._ema_ou_factors = [1.0] * len(self.base_lrs)
            self._main_end_lrs = [None] * len(self.base_lrs)
        ...
        for i, base_lr in enumerate(self.base_lrs):
            ...
            if t < self.warm_steps:
                ...
                # Keep OU factor at 1.0 during warm phase
                self._ou_factors[i] = 1.0
                self._ema_ou_factors[i] = 1.0
            ...
            # Phase 2: Main phase (with optional OU process)
                ...
                if t == self.warm_steps:
                    # Initialize OU factor at 1.0 at start of main phase
                    self._ou_factors[i] = 1.0
                    self._ema_ou_factors[i] = 1.0

                # Compute base schedule LR (check per-group main_decay_ratio override)
                group_decay_ratio = self.optimizer.param_groups[i].get('main_decay_ratio')
                base_schedule_lr = self._compute_main_base_lr(base_lr, t_main, main_decay_ratio=group_decay_ratio)

                # Apply OU process if enabled
                if self.use_ou_process:
                    # Compute sigma for OU process (around 1.0, not base_lr)
                    sigma = self.ou_sigma * math.sqrt(2 * self.ou_theta)

                    # OU update for multiplicative factor: dF = θ(1.0 - F)dt + σdW
                    dt = 1.0
                    drift = self.ou_theta * (1.0 - self._ou_factors[i]) * dt
                    diffusion = sigma * math.sqrt(dt) * self.rng.gauss(0, 1)

                    # Limit max change per step
                    total_change = drift + diffusion
                    max_change_abs = self.ou_max_change
                    if abs(total_change) > max_change_abs:
                        scale = max_change_abs / abs(total_change)
                        total_change *= scale

                    # Update OU factor
                    self._ou_factors[i] = self._ou_factors[i] + total_change

                    # Clip OU factor to reasonable bounds (0.5 to 1.5)
                    self._ou_factors[i] = max(0.5, min(1.5, self._ou_factors[i]))

                    # Apply EMA smoothing if enabled
                    if self.ema_alpha > 0:
                        self._ema_ou_factors[i] = (self.ema_alpha * self._ema_ou_factors[i] +
                                                   (1 - self.ema_alpha) * self._ou_factors[i])
                        ou_factor = self._ema_ou_factors[i]
                    else:
                        ou_factor = self._ou_factors[i]

                    # Apply OU factor multiplicatively
                    lr = base_schedule_lr * ou_factor
                else:
                    # No OU process, just use base schedule
                    lr = base_schedule_lr
            ...
            # Phase 3: Cooldown phase
                ...
                # Reset OU factors during cooldown (no OU in this phase)
                self._ou_factors[i] = 1.0
                self._ema_ou_factors[i] = 1.0
