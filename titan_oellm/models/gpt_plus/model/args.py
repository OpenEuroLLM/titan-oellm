...
class TransformerModelArgs(BaseModelArgs):
    ...
    # Attention Gating parameters (Gated Attention for LLMs, NeurIPS 2025)
    attn_gate_type: str = "none"  # "none", "scalar", "elementwise_dense", "elementwise_lowrank"
    attn_gate_input: str = "x"  # "x" (input) or "xv" (value)
    attn_gate_activation: str = "sigmoid"  # "sigmoid" or "tanh_sq"
    attn_gate_lowrank_dim: int = 64  # Low-rank dimension for elementwise_lowrank
    attn_gate_bias: bool = True  # Whether to use bias in gate linear layers

    @property
    def attn_type(self) -> str:
        """Map use_flex_attn bool to attn_type string for training loop compatibility."""
        return "flex" if self.use_flex_attn else "sdpa"

    def update_from_config(self, job_config: JobConfig, **kwargs) -> None:
        # Core architecture overrides from config (allows YAML configs to be explicit)
        for attr in [f.name for f in fields(self)]:
            if hasattr(job_config.model, attr):
                val = getattr(job_config.model, attr)
                if val is not None:
        ...
        if hasattr(job_config.model, "rope_original_max_position_embeddings"):
            self.rope_scaling_args.original_max_position_embeddings = (
                job_config.model.rope_original_max_position_embeddings
            )
        ...
        if job_config.parallelism.context_parallel_degree > 1 and self.use_flex_attn:
            raise ValueError("FlexAttention is not compatible with CP yet. We are still working on this.")

        # Attention Gating parameters
        if hasattr(job_config.model, 'attn_gate_type'):
            self.attn_gate_type = job_config.model.attn_gate_type
        if hasattr(job_config.model, 'attn_gate_input'):
            self.attn_gate_input = job_config.model.attn_gate_input
        if hasattr(job_config.model, 'attn_gate_activation'):
            self.attn_gate_activation = job_config.model.attn_gate_activation
        if hasattr(job_config.model, 'attn_gate_lowrank_dim'):
            self.attn_gate_lowrank_dim = job_config.model.attn_gate_lowrank_dim
        if hasattr(job_config.model, 'attn_gate_bias'):
            self.attn_gate_bias = job_config.model.attn_gate_bias
