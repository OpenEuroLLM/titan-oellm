    ...
    def __init__(self, model_args: Olmo3CustomModelArgs):
        ...
        if model_args.qk_norm:
            ...
            self.q_norm = nn.RMSNorm(
                q_norm_dim, eps=model_args.norm_eps, elementwise_affine=True
            )
            self.k_norm = nn.RMSNorm(
                k_norm_dim, eps=model_args.norm_eps, elementwise_affine=True
            )
        else:
            self.q_norm = None
            self.k_norm = None
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, model_args: Olmo3CustomModelArgs):
        super().__init__()
        self.n_heads = model_args.n_heads
